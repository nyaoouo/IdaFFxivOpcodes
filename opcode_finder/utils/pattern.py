...
game_data = get_bytes(min_ea, max_ea - min_ea)
...
class _IdaScanner(_IPatternScanner):
    def __init__(self):
    ...
    def search(self, pattern: str | Pattern) -> typing.Generator[tuple[int, list[int]], None, None]:
        ...
        for address, args in pattern.finditer(game_data):
            yield self.min_ea + address, [a + self.min_ea if r else a for a, r in zip(args, pattern.res_is_ref)]


pattern_scanner: _IPatternScanner = _IdaScanner()
